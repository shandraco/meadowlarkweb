...
export default function ResourceFilters({ resources }: { resources: BookableResource[] }) {
  ...
  const chip = (active: boolean) =>
    `px-4 py-2 text-xs tracking-widest uppercase font-normal border transition-colors ${
    ...
    <div className="space-y-8">
      <div className="border border-meadow/10 bg-paper-dark/40 p-5 md:p-6">
        ...
        <div className="flex flex-wrap items-center gap-2 gap-y-3">
          <span className="text-xs text-ink-soft font-normal">Covered:</span>
          {(
            [
          ...
          <div className="flex items-center gap-2 ml-2">
            <span className="text-xs text-ink-soft font-normal">Min guests:</span>
            <input
              type="number"
              min={0}
              value={filters.minCapacity || ""}
              onChange={(e) => setFilters((f) => ({ ...f, minCapacity: parseInt(e.target.value, 10) || 0 }))}
              className="w-16 border border-meadow/20 bg-paper text-ink px-2 py-1 text-sm font-normal outline-none focus:border-meadow"
            />
          </div>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <p className="text-ink-soft font-normal">No spaces match those filters.</p>
      ) : (
          ...
          {filtered.map((r) => (
            ...
            >
              ...
              <div className="p-6 flex-1 flex flex-col">
                ...
                {r.capacity && <p className="text-xs text-ink-soft font-normal mt-1">Up to {r.capacity} guests</p>}
                {r.description && <p className="text-sm text-ink-soft font-normal leading-relaxed mt-4 flex-1">{r.description}</p>}
