        ...
        <AnimatePresence mode="wait">
          {submitted ? (
            <motion.div key="success" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="text-center py-6">
              ...
              <p className="text-ink-soft font-normal text-sm">We&apos;ll reach out before each season you selected.</p>
            </motion.div>
          ) : (
            <motion.form key="form" onSubmit={handleSubmit}>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {SEASON_TOPICS.map((s) => (
                  <button
                    ...
                    type="button"
                    onClick={() => toggle(s.id)}
                    className={`px-4 py-2 text-xs tracking-wide font-normal border transition-all duration-200 ${
              ...
              <div className="flex gap-0">
                <input
                  ...
                  className="flex-1 border border-meadow/20 bg-paper text-ink placeholder:text-ink-soft/40 px-4 py-3 text-sm font-normal outline-none focus:border-meadow transition-colors"
                />
                <button
                  type="submit"
                  disabled={selected.length === 0 || pending}
                  className="px-6 py-3 bg-meadow text-paper text-xs tracking-widest uppercase font-normal hover:bg-meadow-deep transition-colors disabled:opacity-40 shrink-0"
                >
                  {pending ? "…" : "Notify Me"}
                </button>
              </div>
              {error && <p className="text-xs text-sunset font-normal mt-2">{error}</p>}
              {selected.length === 0 && email && (
                <p className="text-xs text-ink-soft font-normal mt-2">Pick at least one season above.</p>
              )}
            </motion.form>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence mode="wait">
          {submitted ? (
            <motion.div key="success" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="text-center">
              ...
              <p className="text-paper/60 font-normal">We&apos;ll reach out before each season you selected. See you at the farm.</p>
            </motion.div>
          ) : (
            <motion.form key="form" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit}>
              <div className="text-center mb-12">
                ...
                <p className="text-xs tracking-widest uppercase font-normal text-wheat mb-4">Remind Me</p>
                <h2 className="font-serif text-4xl md:text-5xl text-paper leading-tight mb-4">
                  Never miss a season
                  <br />
                  <em>at Meadowlark.</em>
                </h2>
                <p className="text-paper/60 font-normal max-w-lg mx-auto">
              ...
              <div className="flex flex-wrap justify-center gap-3 mb-10">
                {SEASON_TOPICS.map((s, i) => (
                  ...
                  >
                    <span className="text-xs tracking-widest uppercase font-normal">{s.label}</span>
                    <span className={`text-xs mt-0.5 font-normal ${selected.includes(s.id) ? "text-ink/70" : "text-paper/40"}`}>
                      {s.when}
                    </span>
                  </motion.button>
                ))}
              ...
              <div className="max-w-md mx-auto space-y-3">
                <div className="flex gap-0">
                  <input
                    ...
                    onChange={(e) => setEmail(e.target.value)}
                    placeholder="your@email.com"
                    className="flex-1 bg-meadow border border-paper/20 text-paper placeholder:text-paper/30 px-5 py-4 text-sm font-normal outline-none focus:border-wheat/60 transition-colors"
                  />
                  <button
                    type="submit"
                    disabled={selected.length === 0 || pending}
                    className="px-6 py-4 bg-wheat text-ink text-xs tracking-widest uppercase font-normal hover:bg-wheat-light transition-colors disabled:opacity-40 shrink-0"
                  >
                    {pending ? "···" : "Notify Me"}
                  </button>
                </div>
                <input
                  ...
                  className="w-full bg-meadow border border-paper/10 text-paper placeholder:text-paper/25 px-5 py-3 text-sm font-normal outline-none focus:border-wheat/50 transition-colors"
                />
                {error && <p className="text-xs text-sunset/80 font-normal text-center">{error}</p>}
                {selected.length === 0 && (
                  <p className="text-xs text-paper/30 font-normal text-center">Select at least one season above.</p>
                )}
                <p className="text-xs text-paper/25 font-normal text-center">
